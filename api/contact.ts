...
export default async function handler(req: any, res: any) {
  ...
  console.log("[CONTATTI] Richiesta ricevuta su Vercel Serverless Function");
  ...
  try {
    ...
    if (typeof body === "string") {
      try {
        ...
        console.error("[CONTATTI] Errore nel parsing del body JSON");
    ...
    if (!RESEND_API_KEY) {
      console.error("[CONTATTI] ERRORE: Variabile d'ambiente RESEND_API_KEY non configurata!");
    ...
    if (error) {
      console.error("[CONTATTI] Errore riscontrato da Resend:", error.name, error.message);
    ...
    console.error("[CONTATTI] Eccezione interna server:", err?.message || err);
